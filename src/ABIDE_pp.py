...
ABIDE = glob.glob('../Dataset/ABIDE/**/*.nii', recursive=True) 
print(len(ABIDE))
...
for name in ABIDE:
    ...
    # sitk.WriteImage(corrected_image, "./PreProcessedData/ABIDE/" + name.split('/')[-1])
...
#     input_image = sitk.ReadImage("./Dataset/trial_ABIDE002-Guys-0828-T1_N4.nii.gz")
...
#     sitk.WriteImage(resampled_image, "./Dataset/trial_resample_ABIDE002-Guys-0828-T1_N4.nii.gz")
...
#     # Load images
#     fixed_image = sitk.ReadImage("./Dataset/MNI/MNI152_T1_1mm.nii.gz")  # MNI template
    moving_image = resampled_image # sitk.ReadImage("./Dataset/trial_resample_ABIDE002-Guys-0828-T1_N4.nii.gz")  # Your T1 MRI
    ...
    sitk.WriteImage(moving_resampled,  "../temp/ABIDEinput_KNN/" + name.split('/')[-1] + ".gz")
    sitk.WriteTransform(final_transform, "../temp/ABIDEmask_KNN/" + os.path.splitext(name.split('/')[-1])[0] + ".tfm")
