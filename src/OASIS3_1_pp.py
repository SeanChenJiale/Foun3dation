...
# Split OASIS 3 into 3 portions to preprocess.
...
OASIS3 = glob.glob('../Dataset/OASIS3/OASIS3_1/**/*.nii.gz', recursive=True) 
print(len(OASIS3))
...
for name in OASIS3:
    ...
    # sitk.WriteImage(corrected_image, "./PreProcessedData/OASIS3/" + name.split('/')[-1])
...
#     input_image = sitk.ReadImage("./Dataset/trial_OASIS3002-Guys-0828-T1_N4.nii.gz")
...
#     sitk.WriteImage(resampled_image, "./Dataset/trial_resample_OASIS3002-Guys-0828-T1_N4.nii.gz")
...
#     # Load images
#     fixed_image = sitk.ReadImage("./Dataset/MNI/MNI152_T1_1mm.nii.gz")  # MNI template
    moving_image = resampled_image # sitk.ReadImage("./Dataset/trial_resample_OASIS3002-Guys-0828-T1_N4.nii.gz")  # Your T1 MRI
    ...
    sitk.WriteImage(moving_resampled,  "../temp/OASIS3_1input_KNN/" + name.split('/')[-1])
    sitk.WriteTransform(final_transform, "../temp/OASIS3_1mask_KNN/" + os.path.splitext(name.split('/')[-1])[0] + ".tfm")
